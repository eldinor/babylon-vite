...
export const createVehicle = (scene, options) => {
  ...
  if (vehicleType === VehicleTypes.default) {
    const vehicleMesh = BABYLON.MeshBuilder.CreateCylinder(
      "cone",
      { height, diameterTop, diameterBottom },
      scene
    );
    vehicleMesh.rotation.x = Math.PI * 0.5;

    const redMat = new BABYLON.PBRMaterial("vmMat");
    redMat.metallic = 0;
    redMat.albedoColor = BABYLON.Color3.Red();
    vehicleMesh.material = redMat;
    ...
    const newVehicleMesh = BABYLON.Mesh.MergeMeshes(
      [vehicleMesh, dodecahedron2],
      true
    );
